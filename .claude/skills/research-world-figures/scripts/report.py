...
and rows that are empty across the board (usually a matching failure, not a
world in which micro-states have no data at all).
"""
...
from pwf_lib import SCOPES, load_master, manifest_path, read_csv
...
def main():
    ...
    for scope in scopes:
        ...
        documented = {e["topic_slug"] for e in manifest if e.get("scope") == scope}

        print("== %s: %d rows, %d topics" % (scope, len(rows), len(topics)))
        for t in topics:
